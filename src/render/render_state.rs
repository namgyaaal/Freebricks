...
use std::sync::Arc;
use crate::{
    common::state::*, render::{queries::Queries, texture::*}
};
use winit::{
    window::Window
};
...
impl RenderState {
    ...
    pub fn resize(&mut self, _width : u32, _height: u32) {
        ...
        let depth_texture = Texture::create_depth_texture(&self.device, &self.config, "depth_texture");
        self.surface.configure(&self.device, &self.config);

        // Need to refactor when more is added 
        match self.pass {
            _ => {
                self.pass = Some(RenderPassData { depth_texture })
            }
        }
    }
